...
std::vector<ArMarker> ar_marker_pose; // for AR tags
...
int main(int argc, char** argv) {
  ...
  // This waits for the block to be placed inside the grippers by a user
  // when the camera sees the object, the grippers close
  while(ar_marker_pose.size() == 0); // while no markers wait here
  ...
  // after the grippers have closed on the object perform task
  while(ros::ok()){
