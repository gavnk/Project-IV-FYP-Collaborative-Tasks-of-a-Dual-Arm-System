...
void HardwareInterface::registerControlInterfaces()
{
  ...
  for (auto iter = dynamixel_.begin(); iter != dynamixel_.end(); iter++)
  {
   ...
   // Two right dummy joints//////////////////////////////////
  hardware_interface::JointStateHandle dummy_state_handle1("right_virtual_roll_joint", &dummy_pos_r[0], &dummy_vel_r[0], &dummy_eff_r[0]);
  joint_state_interface_.registerHandle(dummy_state_handle1);

  hardware_interface::JointStateHandle dummy_state_handle2("right_virtual_yaw_joint", &dummy_pos_r[1], &dummy_vel_r[1], &dummy_eff_r[1]);
  joint_state_interface_.registerHandle(dummy_state_handle2);

  hardware_interface::JointHandle dummy_pos_handle1(joint_state_interface_.getHandle("right_virtual_roll_joint"), &dummy_cmd_r[0]);
  position_joint_interface_.registerHandle(dummy_pos_handle1);

  hardware_interface::JointHandle dummy_pos_handle2(joint_state_interface_.getHandle("right_virtual_yaw_joint"), &dummy_cmd_r[1]);
  position_joint_interface_.registerHandle(dummy_pos_handle2);

  ////////////////////////////////////////////////

   // Two left dummy joints///////////////////////////////////////
  hardware_interface::JointStateHandle dummy_state_handle3("left_virtual_roll_joint", &dummy_pos_l[0], &dummy_vel_l[0], &dummy_eff_l[0]);
  joint_state_interface_.registerHandle(dummy_state_handle3);

  hardware_interface::JointStateHandle dummy_state_handle4("left_virtual_yaw_joint", &dummy_pos_l[1], &dummy_vel_l[1], &dummy_eff_l[1]);
  joint_state_interface_.registerHandle(dummy_state_handle4);

  hardware_interface::JointHandle dummy_pos_handle3(joint_state_interface_.getHandle("left_virtual_roll_joint"), &dummy_cmd_l[0]);
  position_joint_interface_.registerHandle(dummy_pos_handle3);

  hardware_interface::JointHandle dummy_pos_handle4(joint_state_interface_.getHandle("left_virtual_yaw_joint"), &dummy_cmd_l[1]);
...
void HardwareInterface::read()
{
  ...
  for(uint8_t index = 0; index < id_cnt; index++)
  {
    ...
    if (strcmp(name_array[index].c_str(), "left_gripper") == 0 || strcmp(name_array[index].c_str(), "right_gripper") == 0)
  ...
  if (strcmp(interface_.c_str(), "position") == 0)
  {
    for (auto const& dxl:dynamixel_)
    {
     ...
     // if (strcmp(dxl.first.c_str(), "gripper") == 0)
      if (strcmp(dxl.first.c_str(), "left_gripper") == 0 || strcmp(dxl.first.c_str(), "right_gripper") == 0)
